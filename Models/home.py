...
class Home(Model):
    def __init__(self, no_patients, patient_starts, robot_start, patient_paths, patient_histories, governor_conf, robo_battery, time_of_day, robot_character):
        super().__init__()
        ...
        for i in range(no_patients):
            self.stakeholders.append(Patient(id_gen.get_id(), 'patient_' + str(i), self, patient_paths[i], patient_histories[i]))

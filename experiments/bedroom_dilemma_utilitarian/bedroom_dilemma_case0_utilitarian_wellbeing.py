...
# Wait for 20 steps
for i in range(20):
    patient_1_path.append(((8, 11), ''))

# move in the bed and wait for 50 steps and move again
patient_1_path.append(((8, 11), 'move'))

for i in range(50):
    patient_1_path.append(((8, 10), ''))

patient_1_path.append(((8,10), 'move'))
...
server = ModularVisualization.ModularServer(
    ...
    "Home model", {"no_patients": 1, "patient_starts": [patient_1_path[0][0]], "robot_start": (5, 5),
                   "patient_paths": [patient_1_path], "patient_histories": [0], "governor_conf":
                       'experiments/bedroom_dilemma_utilitarian/elder_care_sim_utilitarian_wellbeing.yaml', "robo_battery": 100, "time_of_day": "day", "robot_character": {}}
)
...
#                    patient_paths=[patient_1_path], patient_histories=[0], governor_conf=
#                        'experiments_cmd/bedroom_dilemma_utilitarian/elder_care_sim_utilitarian_wellbeing.yaml', robo_battery=100, time_of_day="day", robot_character={
#         })
#
# for i in range(400):
#     model.step()
